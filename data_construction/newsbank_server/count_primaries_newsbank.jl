...
function count_mentions_in_file(f::String, res::Array{Regex,1})
    ...
    try
        ...
        filter!(i -> occursin(r"congress|senat|us house"i, texts[i]), keep_inds)                  # require mention of Congress / Senate / US House

        sects = sects[keep_inds]
        texts = texts[keep_inds]
        counts.article_count = [count_mentions(p, texts) for p in res]
    catch
    ...
    counts[!,:date] .= dt
    counts[counts.article_count .> 0,:]    ### exclude implicit zeros
...
# regexes to search
primary_res = [r"(primar(y|ies))|(nominat(e|ion|ing))"i]
...
@threads for f in loop_folders
    count_mentions_in_dir(f, primary_res)
end
...
joined_file = "$local_dir/newsbank_primary_mentions.csv"

function read_output(f::String)
    one_table = CSV.File(f) |> DataFrame
    select!(one_table, [:date, :article_count])
